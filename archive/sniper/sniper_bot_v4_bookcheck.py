...
SNIPE_DELAY = 0              # Fire IMMEDIATELY after window end (speed race)
MAX_SNIPE_WINDOW = 12        # Max seconds (tighter — if we haven't filled by +12s, Sharky already won)
MIN_PRICE_MOVE_PCT = 0.01    # 0.01% — only trade clear moves (avoid Binance/Chainlink disagreement)
PAPER_MODE = False
...
class SniperBot:
    ...
    def _get_all_close_prices(self):
        """Fetch close prices for all pre-loaded assets in ONE parallel batch.
        Uses Binance spot price — single HTTP call per asset, <500ms each."""
    ...
    def _snipe_all(self, window_ts):
        ...
        1. Close prices fetched in parallel via Binance spot (~300ms)
        2. Compare with pre-loaded open prices
        3. Fire orders immediately on the WINNER token
        Target: <2 seconds total from window end to order placement."""
        t_start = time.time()

        # Get all close prices in parallel (one Binance call per asset, concurrent)
        close_prices = self._get_all_close_prices()
        t_prices = time.time()
        ...
        for asset, data in self.preloaded.items():
            ...
            close_price = close_prices.get(asset)

            if close_price is None:
                self.log(f"⚠️ {asset.upper()}: No close price")
                continue

            # Direction from pre-loaded open vs fresh close
            ...
            # Check orderbook for winning token
            try:
                r = requests.get(f"{CLOB_BASE}/book?token_id={token_id}", timeout=2)
                book = r.json()
                asks = book.get("asks", [])
                buyable = [(float(a["price"]), float(a["size"])) for a in asks if float(a["price"]) <= self.max_buy_price]
            except Exception as e:
                self.log(f"⚠️ {asset.upper()}: Book fetch failed: {e}")
                continue

            if not buyable:
                self.log(f"⏭️ {asset.upper()}: No asks ≤ ${self.max_buy_price} on {outcome.upper()} (book has {len(asks)} asks)")
                self.stats["misses"] += 1
                continue

            best_price = buyable[0][0]
            total_available = sum(s for _, s in buyable)
            margin = 1.0 - best_price

            if margin < MIN_MARGIN:
                self.log(f"⏭️ {asset.upper()}: Margin {margin:.3f} < {MIN_MARGIN}")
                continue

            # Size the order (min 5 shares per Polymarket)
            max_shares = self.trade_size / best_price
            shares = min(max_shares, total_available)
            shares = max(shares, 5.0)  # Polymarket minimum
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # PHASE 2: Update open price from Binance candle near window end
                # Candle open = the actual open price of the 5m window (most accurate)
                if 3 <= secs_to_end <= 8 and self.preloaded:
                    for asset, data in self.preloaded.items():
                        if data["window_ts"] == current_window:
                            candle_open = self._get_binance_candle_open(
                                ASSETS[asset]["binance"], current_window
                            )
                            if candle_open:
                                data["open_price"] = candle_open
