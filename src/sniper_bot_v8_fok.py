"""
🎯 The Outsiders — SNIPER BOT v7

PRE-CLOSE snipe on Polymarket 5-min up/down markets.
Strategy: BUY the predicted winner ~1-2 seconds BEFORE the window closes,
when the active orderbook still has 14K+ shares at $0.99.

KEY INSIGHT: Post-close, the book empties and orders don't fill.
Pre-close, both UP and DOWN have deep liquidity at $0.99.
Direction is determined by comparing Binance spot vs candle open.
At T-1s with a ≥0.05% move, direction reversal is extremely unlikely.

Assets: BTC, ETH, SOL, XRP (BTC preferred — largest moves)
"""
import time
import json
import os
...
from py_clob_client.clob_types import OrderArgs, MarketOrderArgs, OrderType, PartialCreateOrderOptions, BalanceAllowanceParams
...
ASSETS = {
    ...
    # BTC-only for pre-close snipe — must fire in <1s before window closes
    # ETH/SOL/XRP disabled: sequential orders push them past close
...
SNIPE_BEFORE_CLOSE = 2.0     # Fire 2-3s before close (needs ~0.5s for spot price + order)
MIN_PRICE_MOVE_PCT = 0.04    # 0.04% — Binance 100% accurate at >=0.02%, extra buffer for last-second reversal
DIRECTION_CHECK_WINDOW = 3.0 # Start checking direction this many seconds before close
PAPER_MODE = False
...
class SniperBot:
    ...
    def _preload_window(self, window_ts):
        """Pre-fetch market data and get Binance candle open price for the window."""
        self.preloaded = {}
        for asset, cfg in ASSETS.items():
            market = self._find_market(asset, window_ts)
            if not market or market["closed"]:
                continue
            # Get the ACTUAL candle open price from Binance (most accurate)
            open_price = self._get_binance_candle_open(cfg["binance"], window_ts)
            if open_price is None:
                # Fallback to spot if candle not available yet
    ...
    def _get_spot_prices(self):
        """Fetch current spot prices for all pre-loaded assets — parallel, <500ms."""
        import concurrent.futures

        def fetch_spot(asset_symbol):
            ...
            try:
                ...
                return asset, float(r.json()["price"])
            except Exception:
                return asset, None
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ex:
            for asset, price in ex.map(fetch_spot, tasks):
                if price is not None:
                    results[asset] = price
        return results

    def _snipe_preclose(self, window_ts):
        """PRE-CLOSE SNIPE: Fire orders BEFORE the window closes.
        Uses pre-loaded candle open + fresh Binance spot for direction.
        The active book has 14K+ shares at $0.99 — instant fill."""
        t_start = time.time()

        # Get spot prices for all assets (parallel, <500ms)
        ...
        for asset, data in self.preloaded.items():
            ...
            close_price = spot_prices.get(asset)

            if close_price is None:
                self.log(f"⚠️ {asset.upper()}: No spot price")
                continue
            ...
            if close_price >= open_price:
                ...
                outcome = "down"
            pct_move = abs(close_price - open_price) / open_price * 100
            ...
            # PRE-CLOSE BUY: Active book has 14K+ shares at $0.99
            # Buy at $0.991 to cross the spread and fill immediately (like Sharky)
            best_price = 0.991
            margin = 1.0 - best_price  # $0.01/share profit on resolution

            # Check market is accepting orders
            if market.get("closed") or not market.get("accepting_orders", True):
                self.log(f"⚠️ {asset.upper()}: Market closed or not accepting orders")
                continue
            ...
            time_before_close = window_end - now
            elapsed = now - t_start
            
            self.log(
                f"🎯 SNIPE {asset.upper()} {outcome.upper()} [T-{time_before_close:.2f}s] | "
    ...
    def _place_order(self, token_id, price, size, market):
        try:
            # Use MARKET order with FOK (Fill-Or-Kill) for immediate execution
            # This fills against the active orderbook instead of resting as a limit order
            cost = size * price
            order_args = MarketOrderArgs(
                token_id=token_id,
                amount=round(cost, 2),  # Market orders take USD amount for BUY
                side=BUY,
                price=round(price, 2),
                order_type=OrderType.FOK,  # Fill-Or-Kill = immediate or cancel
            )
            ...
            # create_market_order returns a SignedOrder (not yet posted)
            signed_order = self.client.create_market_order(order_args, options)
            
            # post_order submits it to the CLOB
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # PHASE 2: PRE-CLOSE SNIPE — fire 1-2s BEFORE window ends
                # The active book has 14K+ shares at $0.99. Direction from spot vs candle open.
                # Fire when: SNIPE_BEFORE_CLOSE < secs_to_end <= SNIPE_BEFORE_CLOSE + 1
                # E.g., if SNIPE_BEFORE_CLOSE=1.0, fire when 1s < secs_to_end <= 2s
                if SNIPE_BEFORE_CLOSE < secs_to_end <= (SNIPE_BEFORE_CLOSE + 1.0) and self.preloaded:
                    if any(d.get("window_ts") == current_window for d in self.preloaded.values()):
                        self._snipe_preclose(current_window)

                # Reconcile fill data shortly after window closes
                secs_after_close = now - window_end
                if 3 <= secs_after_close <= 5:
                    self._reconcile_fills()

                # Sleep scheduling — TIGHT near window end, lazy otherwise
                if secs_to_end <= DIRECTION_CHECK_WINDOW:
                    time.sleep(0.05)  # 50ms poll in snipe zone
